        ...
        if chunk_segs:
            ...
            for s in chunk_segs:
                rel_segs.append({
                    ...
                    "end": max(0.0, s["end"] - start),
                    "text": s["text"]
                })
            chunk["segments"] = rel_segs
...
def call_highlight_api(
    ...
    for attempt in range(1, MAX_HIGHLIGHT_API_ATTEMPTS + 1):
        ...
        try:
            ...
            if "highlights" in parsed and isinstance(parsed["highlights"], list):
                highlights = _sanitize_highlights(parsed["highlights"], duration=duration)
                if highlights:
                    return {"highlights": highlights}
                if is_chunk or attempt == MAX_HIGHLIGHT_API_ATTEMPTS:
                    return {"highlights": []}
                last_error = "no valid highlights in response"
            else:
                last_error = "JSON missing 'highlights' array"
